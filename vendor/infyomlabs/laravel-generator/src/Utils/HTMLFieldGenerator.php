...
class HTMLFieldGenerator
{
    public static function generateHTML(GeneratorField $field, $templateType, $localized = false)
    {
        ...
        if (strpos($field->htmlType,'vince.') !== false){
          $htmlType = preg_split('/:/',$field->htmlType)[0];
          $fieldTemplate = get_template($htmlType.$localized, $templateType);

          if (!is_null( $field->data_url) ){
            $fieldTemplate = str_replace(
                '$DATA_URL$',
                $field->data_url,
                $fieldTemplate
